...
@router.post("/v2/analyze", response_model=AudioAnalysisResponse)
async def analyze_audio_v2(file: UploadFile = File(...)):
    """
    Analyze uploaded audio file with enhanced onset detection using spectral flux.

    This V2 endpoint provides improved onset detection that is more musically accurate,
    reducing false positives from vibrato and non-musical transients.

    Args:
        file: Audio file (.wav, .mp3, .flac)

    Returns:
        JSON response with analyzed audio data featuring improved onset detection
    """
    # Validate file extension
    if file.filename is None:
        raise HTTPException(status_code=400, detail="No filename provided")
    ...
    try:
        ...
        # Analyze audio file with enhanced V2 algorithm
        analysis_result = audio_service.analyze_vocal_melody(temp_file_path)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error analyzing audio file with V2 algorithm: {str(e)}"
        )